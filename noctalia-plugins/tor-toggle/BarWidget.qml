...
NIconButton {
  ...
  property string statusTooltip: "Tor"
  ...
  onClicked: {
    Quickshell.execDetached(["sh", "-c", "/home/nope/.local/bin/tor-toggle"])
    statusProc.running = true
  }

  Process {
    id: statusProc
    command: ["/home/nope/.local/bin/tor-status"]
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        try {
          ...
          root.statusTooltip = parsed.tooltip || "Tor"
        } catch (e) {
          root.statusText = "?"
        }
      }
