...
NIconButton {
  ...
  property string statusTooltip: "Twingate"
  ...
  onClicked: {
    Quickshell.execDetached(["sh", "-c", "/home/nope/.local/bin/twingate-toggle"])
    statusProc.running = true
  }

  Process {
    id: statusProc
    command: ["/home/nope/.local/bin/twingate-status"]
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        try {
          ...
          root.statusTooltip = parsed.tooltip || "Twingate"
        } catch (e) {
          root.statusText = "?"
        }
      }
